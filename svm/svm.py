...
####Save parameters and accuracy####
    ...
    np.savetxt('predictions_best_model.txt',
               data_to_save,
               fmt='%.6f %.6f %d',
               header='x_coordinate y_coordinate prediction_probability',  # 列标题
               comments='')  # 避免默认添加注释符
    print("\nThe prediction file has been generated using the optimal model: predictions_best_model.txt")
    plt.tight_layout()
    plt.savefig('svm_decision_boundary_comparison.png', dpi=300)
    plt.show()
